...
double deduct_expenses(struct record_list * list, struct time now) { //根据当前时间扣除当前住院患者的费用
	struct record * temp;
	struct time temp_t;
	int start_time;
	int end_time;
	...
	while (temp != NULL) {
		...
		if (now_time >= start_time && now_time <= end_time) {
			...
			else if (temp->tm.lh->pledge < 120000 && temp->tm.lh->pledge >= 100000) { //押金小于1000
...
int day_minus(int month1, int day1, int month2, int day2) { //计算日期差
	int days1 = day_count(month1, day1);
	int days2 = day_count(month2, day2);
	return days2 - days1;
}

double calcCurrentTurnover(struct record_list * list, struct time now) { //计算体检费用和药物费用，住院费用另外计算
	int now_time;
	int in_time;
	int out_time;
	struct record * temp;
	...
	while (temp != NULL) {
		...
		if (in_time < now_time) {//在日期之前看病，计入检查费、药费
			total_money += temp->tm.check_total_price;
			total_money += temp->tm.medicine_total_price;
			...
			if (out_time > now_time) {//未出院
				...
				if (temp->tm.lh->out_time.hour < 8)total_money -= 20000;
			}
		}
		temp = temp->next;
	}
